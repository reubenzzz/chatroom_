...
const JoinChat: React.FC<JoinChatProps> = ({ onJoin }) => {
  ...
  const handleJoinClick = () => {
    if (!displayName.trim()) {
      setError('Please enter a name to join the chat.');
      return;
    }
    setError(null);
    onJoin(displayName);
  ...
  return (
    <div className="bg-[#1A162E]/80 backdrop-blur-sm p-8 rounded-2xl shadow-xl text-center w-full max-w-sm">
        ...
        <p className="text-gray-400 mb-6">Give yourself a name to start chatting</p>
        ...
        <button
            ...
            className="w-full mt-6 bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700 text-white font-bold py-3 px-4 rounded-lg transition-all duration-300 flex items-center justify-center space-x-2 shadow-lg hover:shadow-indigo-500/50"
        >
